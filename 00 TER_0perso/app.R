...
server <- function(input, output) {
    ...
    output$hist <- renderDataTable({
        data <- data.frame(accueil())
        data %>% filter(hh < 2 || hh > 7) %>%
            mutate(date = as.Date(Horodateur, "%d-%m-%Y")) %>%
            arrange(desc(date)) %>% select(hh,tisaep,Horodateur,status)
        # %>% filter(heure < 2,heure > 7)
